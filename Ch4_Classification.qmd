...
One of the running example for this chapter: The (simulated) `Default` data set which is part of the [online resources](https://www.statlearning.com/resources-second-edition) of our textbook `ISLR`.
![](images/Fig_4_1.png)
...
Linear regression is often not appropriate in the case of a qualitative response. 
...
example, there are three possible diagnoses: 
...
(Both quite a lot of nonsense.) 

If the response variable's values did take on a natural ordering, such as *mild*, *moderate*, and *severe*, and we felt the gap between *mild* and *moderate*
...
However, if we use linear regression, some of our estimates might be outside the $[0, 1]$ interval (see Figure 4.2). 
![](images/Fig_4_2.png)

**Summary:**

(a) A classic regression method cannot accommodate a qualitative response with more than two classes
(b) A classic regression method will not provide meaningful estimates of $Pr(Y |X),$ even with just two classes. 
...
However, $0.5$ this is not the only reasonable classification threshold. For instance, if a company wishes to be conservative in predicting individuals who are at risk for default, then they may choose to use a lower threshold, such as $p(\texttt{balance}) > 0.1.$
...
\Leftrightarrow \frac{p(X)}{1-p(X)} &= e^{\beta_0+\beta_1 X}
\end{align*}
$$
...
* A large odds value indicate a high probability of default. 
...
**Caution:**  The amount that $p(X)$ changes due to a one-unit change in
...
$$
\left(\frac{Pr(Y=k|X=x)}{Pr(Y=K|X=x)}\right)=\beta_{k0} + \beta_{k1} x_1 + \dots +  \beta_{kp} x_p
$$
...
* However, interpretation of the coefficients in a multinomial logistic
