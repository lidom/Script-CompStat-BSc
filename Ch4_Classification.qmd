...
* $252$ of the $333$ default individuals are in correctly labeled.<br> 
  False Negative Rate: $(252/333)\cdot 100\%\approx 75.7\%$  
...
The (Receiver Operating Characteristics curve) **ROC curve** is a popular graphic for displaying the two types of errors (false negative rate and false positive rate) for all possible threshold. 
...
* Ideal ROC curve: Touches the complete y-Axis (from zero to one) and then stays as one for all false positive rates. I.e., has values $TP/P=1$ and $FP/N=0$ for all threshold values.

* Summary statistic: Area Under the (ROC) Curve **AUC**; ideal AUC-value is thus AUC$=1.$
