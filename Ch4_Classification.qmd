...
*  incorrect assignment of an individual who defaults to the no default category (false negatives)
* incorrect assignment of an individual who does not default to the default category (false positives)

A **confusion matrix** shows both types of errors. A perfect confusion matrix has zeros in the off-diagonal entries which show the misclassified individuals (see also Table 4.4):
...
* $9644$ of the $9667$ no default individuals are correctly labeled by LDA.<br>True Negative Rate (TN/N): $(9644/9667)\cdot 100\%\approx 99.8\%$
...
If a "False Negative Event" costs the bank more than a "False Positive Event", then the bank wants/needs to adjust the classifying threshold of the classifier. 

In the two class cases, e.g. $\texttt{default}\in\{\texttt{No}, \texttt{Yes}\},$ the Bayes classifier, and thus also LDA which tries to approximate the Bayes classifier, use a threshold of $50\%$. I.e., a subject $i$ with observation $X=x_i$ is assigned to the `default`$=$`Yes` class if
$$
Pr(\texttt{default}_i=\texttt{Yes}|X=x_i)>0.5
$$

However, if the costs of false negatives, i.e. 

> predicting $\texttt{default}_i=\texttt{No}$, when actually $\texttt{default}_i=\texttt{Yes},$

are higher than the costs of false positives, i.e. 

> predicting $\texttt{default}_i=\texttt{Yes}$, when actually $\texttt{default}_i=\texttt{No},$

then one can consider lowering the classifying threshold. For instance, we might label any customer $i$ with a posterior probability of default above $20\%,$
$$
Pr(\texttt{default}=\texttt{Yes}|X=x_i)>0.2,
$$ 
to the `default`$=$`Yes` class. 

This changes the error rates (shown in the diffusion matrix); compare Table 4.4 (threshold $0.5$) with Table 4.5 (threshold $0.2$).

![](images/Tab_4_5.png)

* Changing the threshold from $0.5$ to $0.2$ ...
  * ... **lowered** the false negative rate from $252/333\approx 75.7\%$ to $138/333\approx 41.4\%.$  
  * ... **increased** the false positive rate from $23/9667\approx 0.2\%$ to $235/9667\approx 2.4\%$
  * ... **increased** the overall error rate from $(23 + 252)/10000\approx 3.73\%$ to $(235 + 138)/10000\approx 3.73\%$
...
The (Receiver Operating Characteristics curve) **ROC curve** is a popular graphic for displaying the 
$$
\texttt{true positive rates}=TP/P\quad \text{(``sensitivity'' or ``power'')}
$$ 
against the 
$$
\texttt{false positive rates}=FP/N \quad \text{(``}1-\text{specificity'' or ``type I error'')}
$$ 
for all possible threshold values (from zero to one). 

![](images/Fig_4_8.png)

* Commonly reported summary statistic: Area Under the (ROC) Curve **AUC**
   * Ideal AUC-value: AUC$=1.$
   * A classifier that performs no better than chance has AUC$=0.5.$
...
* LDA tends to be better than QDA if there are relatively few training observations and so reducing variance is crucial.
* QDA is recommended 
   * if the training set is very large, so that the variance of the classifier is not a major concern, or 
...
Bayes’ theorem states that the probability that a observation with predictor values $X=x$ comes from class $k$ (i.e. the posterior probability), is given by
$$
p_k(x) = Pr(Y=k|X=x)=\frac{\pi_k f_k(x)}{\sum_{l=1}^K\pi_l f_l(x)}.
$$

Estimating the prior probabilities $\pi_1,\dots,\pi_K$ is typically straight forward using 
$$
\hat\pi_k=\frac{n_k}{n},\quad k=1,\dots,K
$$ 

Estimating the $p$ dimensional density functions $f_1(x),\dots,f_K(x)$, however, is more difficult. To make estimation tractable and useful, LDA and QDA assume different parametric families (multivariate normals) for these density functions, 

By contrast, *Naive Bayes* only makes the assumption that the $p$ dimensional components of the predictor are independent from each other. I.e. that the joint density $f_k(x)$ can be written as the product of the $p$ marginal densities $f_{kj}(x_j)$, $j=1,\dots,p$, 
$$
f_k(x) = f_{k1}(x_1)\times f_{k2}(x_2)\times \dots \times f_{kp}(x_p)
$$ 

This is a simplifying assumption that may not be completely correct, but which often leads to very good classification results - particularly, in smallish sample sizes.
...
* (univariate) normal distribution assumptions, i.e. $X_j|Y=k \sim N(\mu_k,\sigma_k)$
