...
where 
$$
\sigma^2=Var(\epsilon)\quad\text{for all}\quad i=1,\dots,n.
$$
...
Using repeated samples form the data generating process defined in `myDataGenerator()`, we can generate multiple estimation results $\hat\beta_0$ and $\hat\beta_1$ of the unknown simple linear regression parameters $\beta_0$ and $\beta_1$ and plot the corresponding empirical regression lines:
```{r}
## Estimation
...
Use the above `myDataGenerator()` function to generate many, for instance $B=10000$, realizations of the artificial data generating process, and based on these $B$ many realizations of $\hat\beta_0$ and $\hat\beta_1:$
$$
...
$$
\begin{align*}
E(\hat\beta_0)&\approx \frac{1}{B}\sum_{b=}^B\hat\beta_{0,b}\\[2ex]
E(\hat\beta_1)&\approx \frac{1}{B}\sum_{b=}^B\hat\beta_{1,b}
\end{align*}
$$
...
**Question:** Are the estimators $\hat\beta_0$ and $\hat\beta_1$ unbiased for the artificial data generating process defined by our `myDataGenerator()` function? 
:::
...
\begin{align*}
\sigma^2 & =Var(\epsilon)\\
\sigma  &  =SD(\epsilon) = \sqrt{Var(\epsilon)}
\end{align*}
...
&=\left[\hat{\beta}_1-2\operatorname{SE}(\hat\beta_0),\;
        \hat{\beta}_1+2\operatorname{SE}(\hat\beta_0)\right] = \hat\beta_0\pm 2\operatorname{SE}(\hat\beta_1)\\[2ex]
...
Let us consider the following standard null $(H_0)$ and alternative hypothesis $(H_1):$ 
$$
\begin{align*}
...
\end{align*}
$$
contains the value $0,$ then we cannot reject the null hypothesis that $\beta_1=0.$

* If, however, the observed realization of the confidence interval does not contain the value $0,$ then we can reject the null hypothesis and adopt the alternative that $\beta_1\neq 0.$

::: {.callout-note}

# Probability of a type I error 

The probability of falsely rejecting the null hypothesis even though the null hypothesis is true (i.e. doing a type I error) is smaller or equal to the chosen significance level $\alpha.$

Note: A type I error (rejecting $H_0$ even though $H_0$ is true) is also called a false positive event. We want that false positives happen only (very) rarely and therefore choose a small significance level $\alpha.$
::: 


#### Test Statistics {-}

Standard errors can also be used to do hypothesis testing based on test statistics. 

Let us again consider the standard null $(H_0)$ and alternative hypothesis $(H_1)$: 
$$
\begin{align*}
H_0:&\;\text{There is no relationship between $Y$ and $X$; i.e. $\beta_1=0$}\\ 
H_1:&\;\text{There is a relationship between $Y$ and $X$; i.e. $\beta_1\neq 0$}
...
$$
t=\frac{\hat\beta_1 - 0}{\operatorname{SE}(\hat\beta_1)}\overset{H_0}{\sim}t_{(n-2)}
$$
The $t$-test statistic is a random variable. Under $H_0,$ i.e. if $H_0$ is true, the $t$-test statistic has a $t$-distribution with $(n-2)$ degrees of freedom. 
...
$$
t=\frac{\hat\beta_j - 0}{\operatorname{SE}(\hat\beta_j)}\overset{H_0}{\sim}t_{(n-p-1)}.
$$
:::
...
\end{align*}
where $t_{obs}$ denotes the observed value of the $t$-test statistic computed from the observed training data, and where $t$ is $t$-distributed with $(n-2)$ degrees of freedom. $P_{H_0}$ means that the probability is computed for the scenario that $H_0$ is true. 
...
::: {.callout-note}
Again, the probability of rejecting the null hypothesis even though the null hypothesis is true is smaller or equal to the chosen significance level $\alpha.$
::: 
...
can be warning signals indicating overfitting. While overfitting typically does not happen in simple linear regression analysis, it can happen in multiple linear regression analysis (see below).

**Cautionary Note Nr 2:** The $R^2$ and \operatorname{RSE} are only based on training data. We have seen in @sec-SL that we usually want to assess the model accuracy using testing data.  
:::
...
denote a training data random sample. I.e.  
$$
(X_{i1},\dots,X_{ip},Y_i)\overset{iid}{\sim}(X,Y),\quad\text{for all}\quad i=1,\dots,n.
$$
Moreover, let 
$$
...
\end{equation*}
This allows us to write the multiple linear regression model more compactly
$$
...
\end{equation*}
denote the $(n\times 1)$ vector containing all response values of the random sample and all error terms of the random sample. Moreover, let
\begin{equation*}
X=\left(\begin{matrix}X_{11}&\dots&X_{1K}\\\vdots&\ddots&\vdots\\ X_{n1}&\dots&X_{nK}\\\end{matrix}\right)
\end{equation*}
denote the $(n\times K)$ matrix containing all predictor values of the random sample. Then
$$
Y=X\beta+\epsilon.
$$
