...
::: {.callout-tip}
In the multiple linear regression model $Y_i=\beta_0+\sum_{j=1}^p\beta_jX_{ij}+\epsilon_i$ (@sec-MultLinReg), the $t$-test for the $j$th predictor has the following null-distribution: 
$$
...
\end{align*}
where, $P_{H_0}$ means that the probability is computed "under $H_0$;" i.e. for the scenario that $H_0$ is true. 
...
$$
depends on the random sample, it is itself a $(p+1)$-dimensional **random variable**. For each realization of the random sample, we observe a realization of the estimator. 

In an actual data analysis, however, we only have *one* realization of the estimators $\hat\beta_0,\hat\beta_1,\dots,\hat\beta_p$ computed from the given dataset (i.e. the observed realization of the random sample).
...
Thus in separate simple linear regressions, the separate effects of 
...
are both (but separately) statistically significant. 

I.e., **in two separate regressions**, we can reject the null hypotheses 
$$
...
only the effect of `newspaper` becomes statistically **insignificant**: 
...
* `radio` has a **positive effect** on `sales`
* `newspaper` has actually **no effect** on `sales`
* But, `newspaper` is correlated with `radio` (cor(`newspaper`,`radio`)=0.3541); see Table 3.5

![](images/Tab_3_5.png)

* When **omitting** `radio` from the multiple regression model, `newspaper` becomes a surrogate for `radio` and we see a spurious effect.  

::: {.callout-caution}
Interpreting statistically significant results as true effects ("a change of $X_j$ by one unit causes on average a change in $Y$ by $\beta_{j}$") is a delectate thing. Even if the $f(X)$ is really so simple that we can write it as a simple or multiple linear regression model, we may miss to include all relevant predictor variables and thus statistically significant results may only be spurious effects due to omitted variables. 
:::
...
**1. Is There a Relationship Between the Response and Predictors at all?**
...
\end{align*}
$$
where the new slope parameter $\tilde{\beta}_2$ is a linear function of $X_2,$ i.e.
$$
...
A linear model that predicts `sales` using `radio`, `TV`, and the interaction, `radio`$\times$`radio`, between the two takes the form 
...
::: {.callout-caution}
The standard formulas for standard errors, confidence intervals, and hypothesis tests in this chapter are based on the **assumption of uncorrelated error terms**.  If in fact there is correlation among the error terms, then the estimated standard errors will tend to **underestimate** the true standard errors. Thus, if the error terms are auto-correlated, we need to take this into account by using so-called auto-correlation robust standard errors. 
:::
...
::: {.callout-caution}
The standard formulas for standard errors, confidence intervals, and hypothesis tests in this chapter are based on the **assumption of error terms with constant variance**. If the error terms are heteroskedastic, we need to take this into account by using so-called heteroskedasticity robust standard errors. 
:::
...
Figure 3.12 shows a clear outlier (observation 20) which, however, has a typical predictor value $x_i$. Such outliers have little effect on the regression fit.
![](images/Fig_3_12.png)


Figure 3.13 shows again a clear outlier (observation 41) which has a predictor value $x_i$ that is *very atypical*. Such outliers are said to have  large **leverage** giving them power to affect the regression fit considerably. 
![](images/Fig_3_13.png)
...
In the case of the **multiple linear regression model**, $h_i$ is the $i$th diagonal of the $(n\times n)$-dimensional "hat-matrix"
$$
H=X(X'X)^{-1}X'.
$$ 
...
Unfortunately, in higher dimensions, KNN often performs worse than simple/multiple linear regression, since non-parametric approaches suffer from the **curse of dimensionality**. Figure 3.20 considers the same strongly non-linear situation as in the second row of Figure 3.19, except that we have added additional noise (i.e. redundant) predictors that are not associated with the response. 
...
* When $p=1$, $50$ data points can provide enough information to estimate $f(X)$ accurately using non-parametric methods since the $K$ nearest neighbors can actually be close to a given test observation $x_0.$ 
* However, when spreading the $50$ data points over a large number of, for instance, $p=20$ dimensions, even the $K$ nearest neighbors tend to become far away from $x_0.$ 
...
::: {.callout-caution}
The `predict()` function with the option `interval = "prediction"` assumes **Gaussian** error terms. If this assumption is not fulfilled, you should not use the prediction interval.
:::
