...
If the assumption of @eq-simpleLinMod is correct, i.e., if the data is actually generated according to the model $f(X) = \beta_0 + \beta_1 X,$ then ordinary least squares estimators 
$$
...
$$ 
is unknown, but can be estimated by 
\begin{align*}
...
**Cautionary Note Nr 2:** The above assessment is only based on training data. We have seen in @sec-SL that we usually want to assess the model accuracy using testing data.  
...
\end{equation*}
denote the $(n\times 1)$ vector containing all response values of the training data and 
\begin{equation*}
X=\left(\begin{matrix}x_{11}&\dots&x_{1K}\\\vdots&\ddots&\vdots\\ x_{n1}&\dots&x_{nK}\\\end{matrix}\right)
\end{equation*}
denote the $(n\times K)$ matrix containing all predictor values of the training data. 
