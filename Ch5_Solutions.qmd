---
title: "Exercises Ch 5: Resampling Methods"
format: html
---
...
k-fold cross-validation is implemented by taking the set of $n$ observations and randomly splitting into $K$ non-overlapping groups of roughly equal group-size (approx. $n/K$). To compute the $k$-th ($k=1,2,...,K$) MSE-estimate, group $k$ is used to as a validation set and the remainder as a training set. The test error is estimated by averaging the $K$ resulting MSE estimates.
...
1. The estimate of the test error rate can be highly variable/instable, i.e. may strongly depending on which observations are included in the training and validation sets. 

2. The validation set error rate may tend to overestimate the test error rate for the actual model fit on the entire data set since the training set has a relatively small sample size.

**LOOCV** is a special case of k-fold cross-validation with $k = n$. Thus, LOOCV is the computationally most intense cross-validation method since the model must be fit $n$ times. Also, LOOCV has higher variance, but lower bias, than k-fold CV. (On the one hand, the LOOCV cross-validation samples are highly correlated; one the other hand, the cross-validation samples are having sample sizes roughly equal to the actual sample size ($n-1\approx n$).
...
Let $\hat{Y} = \hat{f}(X)$ denote the predicti onf of $Y$ for the given $X$. The bootstrap approach works by repeatedly sampling $n$ observations (with replacement) from the original data set--- in total $B$ times, for some large value of $B$, each time fitting a new model, $\hat{f}^*_1, \hat{f}^*_2, ..., \hat{f}^*_B$, and subsequently obtaining the RMSE (Root Mean Squared (Prediction) Error) of the estimates for all $B$ models by computing $\sqrt{\sum^B_{b=1} (\hat{f}^*_b(X)-Y)^2}$
...
```{r}
# We are going to do similar taks several times in this exercise. 
...
We obtain a similar std. error estimate for the balance variable (up to 1 decimal) and for the income variable (up to unity). The std errors computed using the bootstrap approach are smaller.
...
We can observe a quadratic relationship between $Y$ and $X$. Moreover, $X$ ranges from about $-2$ to $2$, while $Y$ ranges from about $-8$ to $2$.
...
The $p$-values only show statistical significance of the linear and the quadratic predicor, which agrees with the CV-results.
