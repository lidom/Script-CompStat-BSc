...
& =\frac{1}{m}\,\sum_{i=1}^m\,E\left[\left(Y_{01}- \hat{f}(x_0)\right)^2\right]\quad(\text{since $Y_{0i}$ and $\hat{f}$ are iid})\\[2ex]
...
& =E\left[\left(f(x_0) + \epsilon_{01} - \hat{f}(x_0)\right)^2\right]\quad(\text{using }\;Y_{01}=f(x_0)+\epsilon_{01})\\[2ex]
...
& =\underbrace{E\left[\left(f(x_0)- \hat{f}(x_0)\right)^2\right]}_{\text{MSE of $\hat{f}(x_0)$}}+0+Var(\epsilon_{01})\\[2ex] 
...
& = \underbrace{Var\left(\hat{f}(x_0)\right) + \left[\operatorname{Bias}\left(\hat{f}(x_0)\right)\right]^2}_{\text{reducible}} + \overbrace{\underbrace{Var\left(\epsilon_{01}\right)}_{\text{irreducible}}}^{=\sigma^2}
\end{align*}
$$
