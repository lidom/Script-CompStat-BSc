...
$$
\textttt{student}\in\{\texttt{Yes}, \texttt{No}\}
$$
...
$$
\ln\left(\frac{P(Y=k|X=x)}{P(Y=K|X=x)}\right)=\beta_{k0} + \beta_{k1} x_1 + \dots +  \beta_{kp} x_p
$$
...
\begin{align*}
P(X = x) 
&= \sum_{l=1}^K \underbrace{P(Y = l)}_{\pi_l} \;\underbrace{P(X_x|Y=l)}_{f_l(x)} \\[2ex]
