...
$$
\frac{1}{{n_{Train}}}\sum_{i=1}^{n_{Train}} I(Y_i\neq \hat{Y}_i)
$$
...
$$
where $\hat{Y}_{i}^{\text{Test}}=\hat{f}(X_{i}^{\text{Test}})$ is the predicted class label that results from applying the classifier $\hat{f}$ (computed from the training data) to the test observation $X_{i}^{\text{Test}}.$ 
...
$$
\hat{P}(Y_i = \text{Person $i$ pays back}|X_i=x_0) \geq 0.9,
$$
...
$$
\mathcal{N}_0=\{i\in\{1,\dots,n\} \;|\; X_i \text{ is the $K$th closest point to }x_0 \text{ or closter}\}.
$$
