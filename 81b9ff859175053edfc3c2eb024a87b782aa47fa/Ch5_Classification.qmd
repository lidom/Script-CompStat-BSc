...
\hat\mu_k    & = \frac{1}{n_k}\sum_{i:y_i=k}x_i\\[2ex]
& = \frac{1}{n} \sum_{i=1}^n x_iI(y_i=k)\\[2ex]
...
\hat\mu_k    & = \frac{1}{n_k}\sum_{i:y_i=k}x_i\\[2ex]
& = \frac{1}{n} \sum_{i=1}^n x_iI(y_i=k)\\[2ex]
\hat{\Sigma} & = \frac{1}{n-pK}\sum_{k=1}^K\sum_{i:y_i=k}(x_i-\hat\mu_k)(x_i-\hat\mu_k)'\\[2ex]
& = \frac{1}{n-K}\sum_{k=1}^K\sum_{i=1}^n(x_i-\hat\mu_k)(x_i-\hat\mu_k)' I(y_i=k),
\end{align*}
$$
where 
...
* LDA requires 
   ...
   * $1\cdot (p+1)/2$ covariance estimates
