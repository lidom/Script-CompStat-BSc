...
The low bias is due to the fact that the prediction $\hat{f}_K(x_0)$ at a given $x_0$ only uses a few (e.g. $K=1$) and very  close $||x_0-X_i||\approx 0$ and thus "good" neighbors $X_i$ for which $\hat{f}_K(x_0)\approx \hat{f}_K(X_i).$
...
The large bias is due to the fact that the prediction $\hat{f}_K(x_0)$ at a given $x_0$ uses observations from a larger neighborhood (e.g. $K=9$) which increases the chance of considering rather distant $||x_0-X_i||\gg 0$ and thus "bad" neigboors with $\hat{f}_K(x_0)\not\approx \hat{f}_K(X_i).$
