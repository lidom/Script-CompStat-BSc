...
$$ 
are independent for all $(i,j)$-paris with $i\neq j=1,\dots,n.$
...
Checking the two linearity conditions for $f(X_i) = X_i'\beta\colon$

* $f(X_i + \tilde{X}_i)=(X_i + \tilde{X}_i)'\beta = X_i'\beta + \tilde{X}_i'\beta=f(X_i) + f(\tilde{X}_i)$
* $f(aX_i)=(aX_i)'\beta=a(X_i'\beta)=a\,f(X_i)$

Thus, the multiple linear regression function $f(X_i)=X_i'\beta$ is a linear function.
...
implies that 
$$
\operatorname{Cov}(\epsilon_i,X_{ik})=0\quad \text{for all}\quad k=0,\dots,q,
$$
since
$$
\begin{align*}
...
&=E(X_{ik} \; \underbrace{E(\epsilon_i |X_k)}_{=0}\,) = 0\quad \text{for all}\quad k=0,\dots,q.
\end{align*}
$$
Thus 
$$
\underbrace{E(\epsilon_i|X_i)=0}_{A} \Rightarrow \underbrace{\operatorname{Cov}(\epsilon_i,X_{ik})=0\quad \text{for all}\quad k=0,\dots,q}_{B}
$$
...
**Classical Example:** Wage regression
...
* $\eta_i$ denotes a well-behaved error term, which is independent of all regressors, which implies 
$$
...
This omitted variable situation violates the exogeneity assumption (by the $\neg B\Rightarrow \neg A$ argument above):
$$
\begin{align*}
...
Use covariance calculation rules: 
...
In this case, the assumption of exogeneity (Assumption 2) is trivially fulfilled, since by the independence between $\epsilon_i$ and $X_i=(X_{i0},\dots,X_{ip})'$ we have that
$$
...
The larger the proportion of the explained variance, the better is the fit of the estimated model $\hat{Y}_i=\hat{f}(X_i)$ to the data $(Y_i,X_i')$, $i=1,\dots,n$. 
...
&\text{Using the sampling error expression $\hat{\beta} - \beta = (X'X)^{-1}X'\epsilon$:}\\[2ex]
...
&=\;\;\;(X'X)^{-1}X'\underbrace{E\left(\epsilon\epsilon'|X\right)}_{=\highlight{Var(\epsilon|X)}}X(X'X)^{-1}
\end{align*}
$$
...
\begin{align*}
&Var(\hat\beta|X)=\\[2ex]
&=(X'X)^{-1}X' \;\highlight{Var(\epsilon|X)}\; X(X'X)^{-1}\\[2ex]
&=(X'X)^{-1}X' \;\highlight{\sigma^2 I_n}\; X(X'X)^{-1}\\[2ex]
...
&Var(\hat\beta|X)=\\[2ex]
&=(X'X)^{-1}X' \;\highlight{Var(\epsilon|X)}\; X(X'X)^{-1}\\[2ex]
&=(X'X)^{-1}X' \;\highlight{\operatorname{diag}(\sigma_1^2,\dots,\sigma_n^2)}\; X(X'X)^{-1}\\[2ex]
...
$$
Typically, the variance terms $\sigma_i^2$ are unknown, but the expression $\left(\sum_{i=1}^n \sigma_i^2 X_i X_i'\right)$ can be estimated using the **HC** estimators above, such that we can work with
$$
...
where
$$
\hat\beta_{j,obs}=\left[(X_{obs}'X_{obs})^{-1}X_{obs}'Y_{obs}\right]_{j}
$$
and (homoskedastic case) 
$$
\widehat{\operatorname{SE}}(\hat\beta_j|X)_{obs}=\sqrt{\left[\hat{\sigma}^2_{obs}(X_{obs}'X_{obs})^{-1}\right]}
$$
