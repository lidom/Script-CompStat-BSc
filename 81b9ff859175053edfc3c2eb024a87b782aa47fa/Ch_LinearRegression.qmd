...
* $\epsilon_i|X_{i2}\sim \mathcal{N}(0,|X_{i2}|\cdot 0.5+1)$

**Problems (Oct. 27, 2025):**

* Plot the true (usually unknown) regression function 
$$
f(x) = E(Y_i|X_i=x),\quad x=(1,x_2,x_3),
$$
where
\begin{align*}
f(x)
& = E(Y_i|X_i=(1,x_1,x_2)^\top)\\ 
& = E(\beta_0 + \beta_1 X_{i1} + \beta_2 X_{i2} + \beta_3 X_{i2}^2 + \epsilon_i|X_i=x)\\
& =   \beta_0 + \beta_1 x_1    + \beta_2 x_2    + \beta_3 x_2^2    + E(\epsilon_i|X_i=x)\\ 
& =   \beta_0 + \beta_1 x_1    + \beta_2 x_2    + \beta_3 x_2^2 
\end{align*}
as a function of $x_2$ by fixing $x_1$ separately to the following values: $x_1\in\{-2,0,2\}.$ 
* Generate data (sample size $n$) by iid resamplings from the above regression model.
* Plot $Y$ separately against $X_{1}$ and $X_{2}$. 
