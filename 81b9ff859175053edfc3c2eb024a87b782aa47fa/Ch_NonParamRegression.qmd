...
(c) Generate $B=1000$ estimation results 
$$
\hat{f}_{K,1}(x_0),\hat{f}_{K,2}(x_0),\dots,\hat{f}_{K,B}(x_0)
$$ 
for $x_0 = 0.2.$ 

* Plot the estimation results $\hat{f}_{K,1}(x_0),\hat{f}_{K,2}(x_0),\dots,\hat{f}_{K,B}(x_0)$ along with their sample mean and the true value of $f(0.2).$
* What is the (approximated) conditional bias of $\hat{f}_{K}(X),$ given $X=x_0.$
* What is the (approximated) variance of $\hat{f}_{K}(X),$ given $X=x_0.$
* What is the (approximated) mean squared estimation error of $\hat{f}_{K}(X),$ given $X=x_0.$




::: {.callout-tip}
# Approximating the local ($x_0$-specific) Mean, Variance, and Mean Squared Estimation Error
The empirical mean 
$$
\frac{1}{B}\sum_{j=1}^B\hat{f}_{K,j}(x_0)\approx E\left[\hat{f}_{K}(X)\mid X=x_0\right]=E\left[\hat{f}_{K}(x_0)\right]
$$
approximates the conditional mean of $\hat{f}_{K}(X),$ given $X=x_0.$

The empirical variance 
$$
\begin{align*}
\frac{1}{B}\sum_{j=1}^B\left(\hat{f}_{K,j}(x_0)-\frac{1}{B}\sum_{j=1}^B\hat{f}_{K,j}(x_0)\right)^2
&\approx Var\left[\hat{f}_{K}(X)\mid X=x_0\right]\\
&=Var\left[\hat{f}_{K}(x_0)\right]
\end{align*}
$$
approximates the conditional variance of $\hat{f}_{K}(X),$ given $X=x_0.$

The empirical mean squared *estimation* error  
$$
\begin{align*}
\frac{1}{B}\sum_{j=1}^B\left(\hat{f}_{K,j}(x_0)-f(x_0)\right)^2
&\approx E\left[\left(\hat{f}_{K}(X)-f(x_0)\right)^2\mid X=x_0\right]\\
&= E\left[\left(\hat{f}_{K}(x_0)-f(x_0)\right)^2\right]
\end{align*}
$$
approximates the conditional mean squared *estimation* error of $\hat{f}_{K}(X),$ given $X=x_0.$
:::
