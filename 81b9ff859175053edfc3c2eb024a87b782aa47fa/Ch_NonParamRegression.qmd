...
1. Compute the distances between $x_0$ and all training data predictor values $X_1,\dots,X_{n_{\text{Train}}}$
$$
d(x_0,X_1),d(x_0,X_2)\dots,d(x_0,X_{n_{\text{Train}}}).
$$
Use these distances to identify the $K$ training data predictor values $X_1,\dots,X_{n_{\text{Train}}}$ that are closest to $x_0$ and collect their indices the index set $\mathcal{N}_0,$ where
$$
\begin{align*}
\mathcal{N}_0 
& =\{i\in\{1,2,\dots,n_{\text{Train}}\} \; |\; d(x_0,X_i)\text{ is one of the $K$ smallest distances}\}
\end{align*}
$$ 
such that 
...
The KNN regression function estimator $\hat{f}_K$ is, of course, only one of many examples. Alternative examples are: 

* Smoothing splines estimator $\hat{f}_\lambda$ (`smooth.spline()`), where $\lambda$ is the smoothing parameter.
* Polynomial regression estimator $\hat{f}_p$, where the polynomial degree $p$ is the smoothing parameter. 
:::
...
&= E\left[(Y^{\text{Test}} - \hat{f}_K(X^{\text{Test}}))^2|X^{\text{Test}}=x_0\right]\\
%&= E\left[(f(X^{\text{Test}})+\epsilon^{\text{Test}} - \hat{f}_K(X^{\text{Test}}))^2|X^{\text{Test}}=x_0\right]\\
&=\underbrace{E\left[\left(f(x_0)-\hat{f}_K(x_0)\right)^2\right]}_{\text{Mean Squared Estimation Error (reducable)}}\;+\;\sigma^2,
\end{align*}
$$
where $\sigma^2=Var(\epsilon^{\text{Test}})=Var(\epsilon)$ denotes the **irreducible** component of the local mean squared (prediction) error $\operatorname{MSE}(x_0,K).$
...
The mean squared **estimation** error of $\hat{f}_K(x_0)$ can be **decomposed** into a **variance** component and a **squared bias** component, i.e.
$$
...
$$
d(x_0, X_i^{\text{Train}})\approx 0\quad\Rightarrow\quad |\hat{f}_K(X_i^{\text{Train}}) - f(x_0)|\approx 0.
$$
...
$$
d(x_0, X_i^{\text{Train}})\gg 0\quad\Rightarrow\quad |\hat{f}_K(X_i^{\text{Train}}) - f(x_0)|\gg 0.
$$
...
**An locally optimal value of $K$** can, in principe, be chosen for every $x_0$-value by choosing that value of $K$ that **minimizes** 
...
However, this requires local $x_0$-specific training data, which usually do not exist in practice. 

**Solution:** Minimize the **global MSE** with respect to $K.$
...
::: {.callout-important}
We do ***not* assume** (as done in @sec-linearRegCh) that the true regression function $f(x)$ is actually a polynomial function. We only assume that the true regression function $f$ is a sufficiently smooth function. Any smooth function $f$ can be **approximated** using a polynomial function (approximation theory).
...
$$
X_i=\frac{i}{n},\quad i=1,\dots,n_{\text{Train}}.
$$
