...
*  The IDE **RStudio** for R is **free** of charge: [www.rstudio.com](http://www.rstudio.com/)
*  The IDE **Positron** for R is **free** of charge: [https://positron.posit.co/](https://positron.posit.co/)
...
namely
$$
X_{1,obs}=`r round(X_vec[1],2)`, 
X_{2,obs}=`r round(X_vec[2],2)`,
X_{3,obs}=`r round(X_vec[3],2)`,
\text{etc.}
$$

Using this observed realization of the random sample, we can estimate the usually unknown mean $\mu=5$ using the empirical mean 
$$
\bar{X} = \frac{1}{n}\sum_{i=1}^n X_i
$$
namely, 
$$
\bar{X}_{obs} = \frac{1}{n}\sum_{i=1}^n X_{i,obs} =`r round(mean(X_vec), 2)`
$$
...
The observed realization of the sample mean 
$$
\bar{X}_{obs}=`r round(myMean(X_vec),2)`
$$ 
is close to the true (usually unknown) population mean $\mu=5.$
...
### Self-Study: Tidyverse Plotting Basics 
...
### Self-Study: Tidyverse Data Wrangling Basics 
...
## Self-Study: Git/GitHub 

For this course, it is not required and not necessary to learn git. The GitHub assignment repository (see below) can be used simply in the browser. However, if you are interested in learning git, you can, of course, do so. 
...
## In Class Coding Exercise (Oct. 20)

#### Marketing Campaign {-}

* Sample size: $n = 5$ stores
...
$$ 
where $M_1,\dots,M_n\overset{\text{iid}}{\sim}M$ are the individual "fixed effects" with
$$
M \sim \mathcal{N}\left(250,2\right)
$$
and $u_{1},\dots,u_{n}\overset{\text{iid}}{\sim}u$ and $v_{1},\dots,v_{n}\overset{\text{iid}}{\sim}v$ are iid error terms with
$$
...
$$
and where $\Delta$
denotes the effect of the marketing campaign. 

Consider the difference sales for each store: 
$$
...
1. Write a function that generates realizations of the iid random sample $\{X_1,\dots,X_n\}$ for a given value of $\Delta$ and of the sample size $n$.
    * Plot your observed data $\{X_{1,obs},\dots,X_{n,obs}\}$ for $\Delta=0$ (no effect) and $\Delta = 1.5$ (positive effect)
2. Estimate the effect of the marketing campaign $\Delta$ using the sample mean $\bar{X}=n^{-1}\sum_{i=1}^nX_i.$ Consider each of the scenarios $\Delta=0$ and $\Delta=1.5$ separately. 
3. Compute the observed confidence interval $\operatorname{CI}_{obs}$ for (the usually unknown) $\Delta.$ Consider each of the scenarios $\Delta=0$ and $\Delta=1.5$ separately. 
4. Repeatedly generate $1000$ realizations of the confidence interval $\operatorname{CI}$ by repeatedly generating $1000$ realizations of the iid random sample $\{X_1,\dots,X_n\}$.
    * What is the relative frequency of $\operatorname{CI}$ realizations containing $\Delta=0$, when the data is generated under the $\Delta=0$ scenario? 
    * What is the relative frequency of $\operatorname{CI}$ realizations containing $\Delta=0$, when the data is generated under the $\Delta=1.5$ scenario?  
