...
$$
where for all $i=1,\dots,n=50$:

* $\varepsilon_{i}\sim N(0,2^2)$ is the error term
...
We still need to simulate realizations of the dependent variable $y_i$. Remember that $y_i=\beta_1 x_{1i}+\beta_1 x_{2i}+\beta_3 x_{3i}+\varepsilon_{i}$. That is, we only need realizations from the error terms $\varepsilon_i$ in order to compute the realizations from $y_i$. 

```{r}
## Generate realizations of the error term
...
```{r}
## Generate realizations of the dependent variable:
...
You extend the function `myOLSFun()` for the computation of the covariance matrix of the slope-estimates, several measures of fits (e.g. R$^2$ and adj.-R$^2$), t-tests, etc.
