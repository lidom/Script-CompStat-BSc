...
$$
the models with $k$ parameters, it fails to select the best fitting model with different numbers of parameters. Indeed, one can show that the "fit" of the model can be always increased by adding more predictors; i.e. $\operatorname{RSS}_k\geq \operatorname{RSS}_{k'}$ for $k<k'.$ 


Thus, we need alternative criteria to choose between models with different numbers of regressors $k\neq k'$. One option is to choose the model with the lower Cross-Validation (CV) error (see @sec-resamplingmethods) which however can be computationally expensive. Alternatively, we can use information criteria such as Mellow's $C_p$, Akaike Information Criterion (AIC), Bayesian Information Criterion (BIC), and adjusted
$R^2$. These are discussed in more detail in Chapter 6. We can also deter-


 which are fast to compute (no splits into training and test data needed). 
...
is the sample variance of the residuals from the large model @eq-modelLarge with all $p$ predictors, and where $\operatorname{RSS}_k$ are the RSS of the estimated model $\widehat{\mathcal{M}}_{\mathcal{I}_k}(X)$ with $k\leq p$ predictors. Under certain regularity assumptions, one can show that $C_p$ is an unbiased estimate of the test MSE. Thus, the best model is the one with the lowest $C_p$ value. 
...
$$
where $\hat\sigma^2$ is defined as in @eq-MCp. Note that the above formula for the AIC omits constants that are irrelevant for choosing between models. Hence for least squares the AIC and Mellow's $C_p$ are proportional to each other, and thus lead to the same model choices. 
...
2. For $k=1,2,\dots,p:$
  ...
  (b) Pick the "best" among these $\binom{p}{k}$ models, and call it $\mathcal{M}_k$. Here "best" is defined as having smallest RSS, or equivalently larges $R^2.$ 
