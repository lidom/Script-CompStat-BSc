...
$${#eq-MCp}
where 
$\hat\sigma^2=\frac{1}{n-p}\sum_{i=1}^n(y_i - \widehat{\mathcal{M}}_{p}(X))$ 
...
Mellow's $C_p,$ AIC, and BIC have rigorous theoretical justifications. The $\operatorname{adjusted } R^2$ is not es well motivated by statistical theory. 
...
> $||x||_2\sqrt{\sum_{j=1}^px^2}$ denotes the $\ell_2$ (or Euclidean) norm of a $p$-dimensional vector. 
...
round(
c(coef(lm_obj_1)[2] / c, coef(lm_obj_2)[2]),
digits = 1)
## comparing the RSS's:
c(RSS_1, RSS_2)
```

By contrast, the ridge regression criterion in @eq-RidgeCrit is ***not* scale invariant** since scaling the predictors will lead (as seen above) to a rescaling of the coefficient estimates and thus affects the penalty term $\lambda\sum_{j=1}^p\beta_j^2.$ Scaling one or more predictors will generally affect *all* ridge regression estimates $\hat\beta_{0,\lambda}^R,\dots,\hat\beta_{p,\lambda}^R.$ Therefore, it is best to apply ridge regression **after standardizing the predictors**, using the formula
$$
...
Figure 6.5 illustrates this, using a simulated data set containing $p = 45$
predictors and $n = 50$ observations. When the number of variables $p$ is almost as large as the number of observations $n$, as in the example in Figure 6.5, the least squares estimates will be extremely variable. 
...
Moreover, ridge regression also has substantial computational advantages over best subset selection, which requires searching through $2p$ models. In contrast, ridge regression models only need to be fitted once for each  candidate tuning parameter $\lambda.$
...
While ridge regression is able to show which predictors are of major vs. minor relevance for predicting the outcome, none of the predictors is actually removed from the model equation since none of the coefficients is exactly set to zero (unless in the limit $\lambda = \infty$).
...
Lasso yields sparse models—that is, models that involve only a subset of the variables which are much easier to interpret. 
...
Once can show that the lasso and ridge regression coefficient estimates solve 
$$
\min_{\beta}\left\{\sum_{i=1}^n\left(y_i - \beta_0 - \sum_{j=1}^p\beta_jx_{ij}\right)^2\right\}\quad\text{s.t.}\quad \sum_{j=1}^p|\beta_j|\leq s
$${#eq-LassoCrit2}

and
$$
\min_{\beta}\left\{\sum_{i=1}^n\left(y_i - \beta_0 - \sum_{j=1}^p\beta_jx_{ij}\right)^2\right\}\quad\text{s.t.}\quad \sum_{j=1}^p\beta_j^2\leq s,
$${#eq-RidgeCrit2}

respectively. That is, for every $\lambda$ in @eq-RidgeCrit there is some $s$ such in @eq-RidgeCrit2 that will lead to the same ridge coefficient estimates. Likewise, for every $\lambda$ in @eq-LassoCrit there is some $s$ such in @eq-LassoCrit2 that will lead to the same lasso coefficient estimates.

In the case of two $(p=2)$ predictors, one can interpret @eq-LassoCrit2 and @eq-RidgeCrit2 graphically. For $p=2$, ...

* the lasso coefficient estimates have the smallest RSS out of all points that lie within the diamond defined by
$|\beta_1|+|\beta_2|\leq s.$ 

* the ridge coefficient estimates have the smallest RSS out of all points that lie within the circle defined by
$\beta_1^2+\beta_2^2\leq s.$ 

The key idea also applies to cases $p>2.$

The tuning parameter $s$ acts like a budge constraint for how large the penalty term can be.  

![](images/Fig_6_7.png)


@eq-LassoCrit2 and @eq-RidgeCrit2 reveal a close connection between the
lasso, ridge regression, and best subset selection. Consider the problem
$$
\min_{\beta}\left\{\sum_{i=1}^n\left(y_i - \beta_0 - \sum_{j=1}^p\beta_jx_{ij}\right)^2\right\}\quad\text{s.t.}\quad \sum_{j=1}^pI(\beta_j\neq 0)\leq s,
$${#eq-BestSubSetCrit}
where $I(\beta_j\neq 0)=1$ if $\beta_j\neq 0$ and zero else.

Thus @eq-BestSubSetCrit amounts to finding a set of at most $s$ many coefficient estimates such that RSS is as small as possible. The problem of @eq-BestSubSetCrit is equivalent to Step 2 in Best Subset Selection (Algorithm 6.1) for a given number of predictors $s$ (or $k$ in Algorithm 6.1).$[^1] 

[^1]: The choice of $s$ (or $lambda$) is then critical and needs to be done using, for instance, CV. 

Therefore, we can interpret ridge regression and lasso (even more so) as computationally feasible version of Best Subset Regression. 


Figure 6.7 actually shows why lasso is able to perform true variable selection, by contrast to ridge regression. The ellipses show coefficient estimates leading to equal RSS values. While the $\ell_1$ geometry of lasso's "budge constraint" allows to set a coefficient value to zero (here $\beta_1$), the $\ell_2$ geometry of ridge regression's budget constraint only allows to shrink a coefficient value towards zero.    
...
Each value of the tuning parameters $\lambda$ in @eq-RidgeCrit and @eq-LassoCrit or of the tuning parameters $S$ in @eq-LassoCrit2 and @eq-RidgeCrit2 represent a new more or less flexible model---similarly to the tuning parameter $k\leq p$ (number of predictors included) in Best Subset Selection. Thus, as in Best Subset Selection we need to be carful when comparing models of different flexibility. 
...
$${#eq-LinMod_Trans_M}
where $z_{1m},\dots,z_{nm}$ are observed measurements from $Z_m,$ and where $\theta_0,\dots,\theta_M$ are the regression coefficients that we estimate using least squares. 

If the constants $\phi_{jm}$, $j=1,\dots,p$ and $m=1,\dots,M$, are chosen wisely, then such dimension reduction (from $p$ to $M$) approaches can  outperform (lower test MSE) least squares regression. 

Notice that the linear combination of the $M$ transformed predictors in @eq-LinMod_Trans_M can be rewritten as a linear combination of the $p$ original predictors:
$$
\begin{align*}
\sum_{m=1}^M\theta_m z_{im} 
&= \sum_{m=1}^M\theta_m \overbrace{\left(\sum_{j=1}^p\phi_{jm}X_{j}\right)}^{z_{im}}\\
&= \sum_{j=1}^p\underbrace{\left(\sum_{m=1}^M\theta_m \phi_{jm}\right)}_{=\beta^M_j}X_{j}.
\end{align*}
$$
Hereby, 
$$
...
* If $M=p$ (no dimension reduction), and all the $Z_m$ are linearly independent[^2], then @eq-RegulBeta_M poses no constraints (no regularization) and $\beta_j^M=\beta_j.$
* If $M<p$ (dimension reduction), @eq-RegulBeta_M serves to constrain the estimated $\beta_j$ coefficients $(\beta_j^M\neq \beta_j).$   

[^2]: Linear independence of all the $Z_m$ is required anyways to compute the least squares estimates of $\theta_m$.
...
Principal components analysis (PCA) is a popular approach for deriving a low-dimensional $(M<p)$ set of features $Z_1,Z_2,\dots,Z_M$ from a large set of variables $X_1,X_2,\dots,X_p.$ 
...
The first principal component of a set of variables $X_1,X_2,\dots,X_p$ is the normalized linear combination
$$
...
> The vector $\phi_1=(\phi_{11},\dots,\phi_{p1})'$ is called the vector of **loadings** $\phi_{11},\dots,\phi_{p1}$ of the first principal component $Z_1.$ 

To estimate the coefficients $\phi_1=(\phi_{11},\dots,\phi_{p1})',$ we need observed measurements of $X_1,\dots,X_p,$ which we can collect in the $n\times p$ matrix $\mathbf{X},$ such that 
$$
\mathbf{X} = \left[
    \begin{matrix}
...
\right]
$$
where the columns in $\mathbf{X}$ are centered $(x_{ij}=x^{original}_{ij}-\frac{1}{n}\sum_{i=1}^nx_{ij}),$ since we are only interested in maximizing variances. We then look for the linear combination of the sample feature values of the form
$$
z_{i1} = \phi_{11}x_{i1} + \dots + \phi_{p1}X_{ip},
$$
that has larges sample variance subject to the side constraint that  $||\phi_1||_2^2=1.$ In other words, the first principal component loading vector $\phi_1=(\phi_{11},\dots,\phi_{p1})'$ is determined as the solution of the optimization problem 
$$
...
$$ 
where $\frac{1}{n}\sum_{i=1}^nz_{i1}^2$ equals the sample variance of $z_{11},\dots,z_{n1}$ since the average of $z_{11},\dots,z_{n1}$ is zero, because of the centered $x_{ij}.$

There is a nice geometric interpretation for the first principal component. The loading vector $\phi_1=(\phi_{11},\dots,\phi_{p1})'$ defines a direction in feature space along which the data vary the most. If we project the $n$ data points 
$$
x_1=(x_{11},\dots,x_{1p})', \dots, x_n=(x_{n1},\dots,x_{np})'
$$
onto this direction, the projected values are the **principal component scores** $z_{11},\dots,z_{n1}$ themselves, i.e
$$
z_{i1} = \phi_1'x_i = \sum_{j=1}^p \phi_{j1}x_{ij}. 
$$

The case $p=2$ is simple to visualize. Figure 6.14 displays the first principal component loading vector (green solid line) on an advertising data set. 

![](images/Fig_6_14.png)

The left panel in Figure 6.15 displays the projection of the $i$th ($i=1,\dots,n$) data vector $x_i=(x_{i1},\dots,x_{ip})'$ onto the loading vector $\phi_1=(\phi_{11},\dots,\phi_{p1})'$ of the first principal component. These projections map $x_i$ (purple circles 🟣) to $z_{i1}$ (black crosses $\mathbf{\times}$) for each $i=1,\dots,n.$

![](images/Fig_6_15.png)


After the first principal component $Z_1$ of the features has been determined, we can find the second principal component $Z_2.$ The second principal component is the linear combination of $X_1,X_2,\dots,X_p$ that has maximal variance out of all linear combinations that are **uncorrelated** with $Z_1.$ 
...
$$
where $\phi_2=(\phi_{12},\dots,\phi_{p2})'$ is the second principal component loading vector. 

Constraining $Z_2$ to be uncorrelated with $Z_1$ is equivalent to constraining $\phi_2$ to be orthogonal to $\phi_1$, i.e.
$$
...
\text{and}&\quad \sum_{j=1}^p\phi_{j\ell}\phi_{jm}=0\quad\text{for all}\quad \ell<m,
\end{align*}
$$ 
...
We see that the smallest cross-validation error occurs when $M=18$ components are used. This is barely fewer than $M=19$, which amounts to simply performing least squares, because when all of the components are used in PCR no dimension reduction occurs. However, from the plot we also see that the cross-validation error is roughly the same when only one component is included in the model. This suggests that a model that uses just a small number of components might suffice.
 ...
 The `summary()` function also provides the *percentage of variance explained* in the predictors and in the response using different numbers of components. This concept is discussed in greater detail in Chapter 12.
