...
One option is to choose the model with the lowest Cross-Validation (CV) error (@sec-resamplingmethods). In cases, where CV is computationally too expensive, we can use information criteria such as Mellow's $C_p$, Akaike Information Criterion (AIC), Bayesian Information Criterion (BIC), and adjusted $R^2$ that try to approximate the test error of a model. 
...
2. For $k=1,2,\dots,p:$
  2.a Fit all $\binom{p}{k}$ models that contain exactly $k$ predictors.
  2.b Pick the "best" among these $\binom{p}{k}$ models, and call it $\widehat{\mathcal{M}}_k$. Here "best" is defined as having smallest $\operatorname{RSS}_k$ (or equivalently largest $R^2.$) 
...
* Step 2 identifies the best model (on training data) for each subset size $k$, and thus reduces the model selection problem from $2^p$ models to $p+1$ models. 
...
Best subset selection (Algorithm 6.1) can be computationally (too) expensive. 
