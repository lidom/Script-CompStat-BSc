...
$${#eq-LinMod_Trans_M}
where $z_{im}$, $i=1,\dots,n$, are observed measurements from $Z_m,$ and where $\theta_0,\dots,\theta_M$ are the (unknown) regression coefficients that we estimate using least squares. 
...
* If $M=p$ (no dimension reduction), and if all the $Z_m$ are linearly independent[^2], then @eq-RegulBeta_M poses no constraints and $\beta_j^M=\beta_j.$
...
> The vector $\phi_1=(\phi_{11},\dots,\phi_{p1})'$ is called the vector of **loadings**. An in absolute values large loading parameter $\phi_{1j}$ means that the $j$ predictor $X_i$ contributes much to the first principal component $Z_1.$ 
...
\right],
$$
where the columns in $\mathbf{X}$ are centered, i.e. 
$$x_{ij}=x^{orig}_{ij}-\bar{x}^{orig}_j
\quad\text{with}\quad 
\bar{x}^{orig}_j = \frac{1}{n}\sum_{i=1}^nx_{ij}^{orig}
$$
such that $\bar{x}_{j} = \frac{1}{n}\sum_{i=1}^nx_{ij}=0.$
<!-- $$
\bar{x}_{j} = \frac{1}{n}\sum_{i=1}^n \overbrace{(x^{orig}_{ij}-\bar{x}_j)}^{=x_{ij}} =\bar{x}_j-\bar{x}_j= 0. 
$$ -->
...
$$
\begin{align*}
\bar{z}_1=\frac{1}{n}\sum_{i=1}^nz_{i1} 
& = \phi_{11} \frac{1}{n}\sum_{i=1}^n x_{i1} + \dots + \phi_{p1} \frac{1}{n}\sum_{i=1}^nx_{ip}\\
& = \phi_{11}\bar{x}_{i1} + \dots + \phi_{p1}\bar{x}_{p} = 0.
\end{align*}
$$
Therefore, the formula for the sample variance of $z_{i1}$, $i=1,\dots,n$, simplifies
$$
\frac{1}{n}\sum_{i=1}^n\left(z_{i1}-\bar{z}_1\right)^2=\frac{1}{n}\sum_{i=1}^nz_{i1}^2.
$$
We then look for those parameter $\phi_{11},\dots,\phi_{p1}$ in 
$$
z_{i1} = \phi_{11}x_{i1} + \dots + \phi_{p1}x_{ip},
$$
that maximize the sample variance of $z_{i1}$, $i=1,\dots,n$, subject to the side constraint that $||\phi_1||_2^2=1.$ 


In other words, the first principal component loading vector $\phi_1=(\phi_{11},\dots,\phi_{p1})'$ is determined as the solution of the optimization problem 
$$
...
There is a nice geometric interpretation for the first principal component. The loading vector $\phi_1=(\phi_{11},\dots,\phi_{p1})'$ defines a direction in feature space along which the data vary the most. If we project the $n$ many $p$-dimensional data points 
$$
x_1=(x_{11},\dots,x_{1p})', \dots, x_n=(x_{n1},\dots,x_{np})'
$$
onto this direction $\phi_1=(\phi_{11},\dots,\phi_{p1})'$, the projected values are the **principal component scores** $z_{11},\dots,z_{n1}$ themselves, i.e
$$
...
The left panel in Figure 6.15 displays the projection of the $i$th, $i=1,\dots,n,$ data vectors $x_i=(x_{i1},\dots,x_{ip})'$ onto the loading vector $\phi_1=(\phi_{11},\dots,\phi_{p1})'$ of the first principal component. These projections map $x_i$ (purple circles 🟣) to $z_{i1}$ (black crosses $\mathbf{\times}$) for each $i=1,\dots,n.$
