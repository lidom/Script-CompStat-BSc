...
Moreover, if a certain value $k=1,\dots,K$ maximizes $p_k(x),$ then the same $k$ maximizes also $\log(p_k(x))$ since the $\log$-function is a strictly monotonic increasing formation. 
...
$$
E(X|Y=k)=\mu_k=\left(\begin{matrix}
  \mu_{k1}\\
  \mu_{k2}\\
  \vdots\\
  \mu_{kp}
  \end{matrix}\right),
$$
and $(p\times p)$ covariance matrix that is common for all classes $k=1,\dots,K,$ i.e.
$$
Cov(X|Y=1)=\dots=Cov(X|Y=K)=Cov(X),
$$ 
where 
$$
Cov(X)=
