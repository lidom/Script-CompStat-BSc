...
2. The multivariate, $p+1$ dimensional, random vectors $(X_i,Y_i),$ $i=1,\dots,n,$ have all the same distribution as $(X,Y)$ from @eq-GRegMod, i.e.
$$
(X_i,Y_i)\sim(X,Y)
$$
...
\{(x_1,y_1),(x_2,y_2),\dots,(x_n,y_n)\}.
$$
These $n$ observations are called **training data** because we will use these training observations to train, i.e., compute and estimate $\hat{f}$ of the unknown $f.$ 
...
Let 
$$
\{(X_{01},Y_{01}),(X_{02},Y_{02}),\dots,(X_{0n},Y_{0m})\},
$${#eq-trainingsample}
...
The new test data random sample @eq-trainingsample 

* is independent of the training data random sample @eq-randomsample
* has the same distribution as the training data random sample @eq-randomsample
...
$$
denote the set of $m$ **test data points** $y_{01},\dots,y_{0m}$ for one given predictor value $x_0$. This type of $x_0$-specific test data is a realization of a random sample conditional on $X=x_0$
$$
(x_{0i},Y_{0i})\overset{\text{iid}}{\sim}(X,Y)|X=x_0,\quad i=1,\dots,m.
$$ 
...
\{(x_{01},y_{01}),(x_{02},y_{02})\dots,(x_{0m},y_{0m})\}
$$
denote the set of $m$ test data points with different predictor values $x_{01},\dots,x_{0m}$ in the range of $X$. Then, the **global test MSE** is given by,
\begin{align*}
...
$$
estimates the variance of the error term $Var(\epsilon)$, i.e., equals the **irreducible error component** of the mean squared (prediction) error. 


<br>

Figures 2.9 shows training and test MSEs for smoothing spline estimates $\hat{f}$ in the case of 

* a moderately complex $f$ 
* a moderate signal-to-noise ratio $Var(f(X))/Var(\epsilon)$

![](images/Fig_2_9.png)

<br>

Figures 2.10 shows training and test MSEs for smoothing spline estimates $\hat{f}$ in the case of 
...
Figures 2.11 shows training and test MSEs for smoothing spline estimates $\hat{f}$ in the case of 

* a moderately complex $f$
* a very large signal-to-noise ratio $Var(f(X))/Var(\epsilon)$
...
* Let $\hat{f}$ be estimated from the training data
* Let 
$$
Y_0=f(x_0)+\epsilon
$$ 
