...
**Step 1:** Estimation of the polynomial regression model, e.g., for $p=2$ using the training set:
...
**Step 2:** Validation of the polynomial regression model by computing the test mean squared (prediction) error (**test MSE**) using the validation set:
...
Repeating Step 1 and Step 2 for a series of polynomial degrees $p=1,\dots,10$
...
Repeating the above computations for multiple random splits of the dataset into training and validation sets
...
@fig-VA2 shows that the validation set approach can be highly variable. The selected polynomial degrees (minimal test MSE) strongly vary across the different random splits and thus depend on the data included in the test and validation sets. 

A particular problem with the validation set approach are small sample sizes. Both the training and the validation set have reduced sample sizes 
$$
n_{Train}<n\quad\text{and}\quad n_{Valid}<n
$$ 
which leads to **increased estimation uncertainties when estimating the test MSE values**.


*Cross-validation* is a refinement of the validation set approach that addresses this issue. 

### (Ch. 5.1.2) Leave-One-Out Cross-Validation {-}
