...
Repeatedly generating new datasets by sampling new observations from the true population distribution, for instance, $B=1000$ many times, would allow us to compute $B=1000$ estimates  
$$
...
```{r chunk13}
set.seed(7)
alpha_fn(Portfolio, sample(x=1:100, size=100, replace = TRUE))
```
...
Since the quadratic model provides a good fit to the data (Figure 3.8), there is now a better correspondence between the bootstrap estimates and the standard estimates of ${\rm SE}(\hat{\beta}_0)$, ${\rm SE}(\hat{\beta}_1)$ and ${\rm SE}(\hat{\beta}_2)$.
